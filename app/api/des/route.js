...
export async function POST(req) {
  try {
    ...
    if (action === "encrypt") {
      result = CryptoJS.DES.encrypt(text, key, {
      ...
      if (!result) throw new Error("Decryption failed. Invalid input.");
    } else {
      return NextResponse.json(
...
//       const textBinary = stringToBinary(text).padEnd(64, "0");
...
//       const result = binaryToString(resultBinary);
